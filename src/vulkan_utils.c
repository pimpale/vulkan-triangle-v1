...
ErrVal new_Instance(VkInstance *pInstance, const uint32_t enabledLayerCount,
  ...
  // and glfw)
  size_t allExtensionCount = enabledExtensionCount;
...
ErrVal new_VertexDisplayPipeline(VkPipeline *pGraphicsPipeline,
  ...
  bindingDescription.stride = sizeof(Vetex);
...
ErrVal new_GlfwWindow(GLFWwindow **ppGlfwWindow, const char *name,
  ...
  *ppGlfwWindow =
      glfwCreateWindow(dimensions.width, dimensions.height, name, NULL, NULL);

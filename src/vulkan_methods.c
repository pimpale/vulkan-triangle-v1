...
/* Preconditions: ppEnabledExtensionNames
 *
 */
bool extensionsAvailable(uint32_t enabledExtensionCount,
                         const char *const *ppEnabledExtensionNames) {
  uint32_t extensionCount = 0;
  vkEnumerateInstanceExtensionProperties(NULL, &extensionCount, NULL);
  VkExtensionProperties *arr =
      malloc(extensionCount * sizeof(VkExtensionProperties));
  char **ppInstanceExtensionNames =
      malloc(extensionCount * sizeof(char *));
  if (!arr || !ppInstanceExtensionNames) {
    printError(errno);
    hardExit();
  }
  ...
  uint32_t matchnum = 0;
  findMatchingStrings(ppInstanceExtensionNames, extensionCount, 
				  ppEnabledExtensionNames, enabledExtensionCount,
				  NULL, 0, &matchnum);

  for (uint32_t i = 0; i < extensionCount; i++) {
    ppInstanceExtensionNames[i] = arr[i].extensionName;
  }
  
  free(arr);
  free(ppInstanceExtensionNames);
  return matchnum == enabledExtensionCount;
}

bool deviceExtensionsAvailable(VkPhysicalDevice device,
...
bool deviceLayersAvailable(VkPhysicalDevice device, uint32_t enabledLayerCount,
...
bool layersAvailable(uint32_t enabledLayerCount,
                     const char *const *ppEnabledLayerNames) {
  uint32_t layerCount = 0;
